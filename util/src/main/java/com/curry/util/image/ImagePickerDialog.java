 ...
 * @program: Toolt
 * @author: 陈博文
 * @create: 2022-08-06 17:12
 * @description: 选择图片的dialog
 **/
public class ImagePickerDialog extends Dialog implements View.OnClickListener{
    ...
    public ImagePickerDialog(Context context, int theme) {
        super(context, theme);
        this.mContext = context;
        initDialog();
    }
        ...
        setOnKeyListener(new OnKeyListener() {
            ...
            public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_UP) {  //返回键监听
                    hideDialog();
                    mListener.onItemClick(ImagePicker.CANCEL);//返回键关闭dialog
                }
                return false;
            }
        });
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (isOutOfBounds(getContext(), event)) {
            hideDialog();
            mListener.onItemClick(ImagePicker.CANCEL);//触摸dialog外部关闭dialog
        }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btn_camera) {
            hideDialog();
            mListener.onItemClick(ImagePicker.CAMERA);
        } else if (v.getId() == R.id.btn_album) {
            hideDialog();
            mListener.onItemClick(ImagePicker.ALBUM);
        } else if (v.getId() == R.id.btn_cancel) {
            hideDialog();
            mListener.onItemClick(ImagePicker.CANCEL);//点击“取消”关闭dialog
        }
    }
